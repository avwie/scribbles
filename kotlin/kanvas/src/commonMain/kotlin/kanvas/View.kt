...
fun render(model: Model) = svg<Message> {
    with(model.grid) {
        "width" by width()
        "height" by height()
        "viewBox" by "0 0 ${width()} ${height()}"
    }
...
fun View.layer(name: String, block: Block) = "g" {
    "class" by "layer $name"
    block()
}
    ...
    if (grid.visible) {
        repeat(grid.rows + 1) { i ->
            "line" {
                "x1" by 0
                "x2" by grid.width()
                "y1" by i * grid.cellHeight
                "y2" by i * grid.cellHeight
                "stroke" by "black"
                "stroke-dasharray" by 2
            }
        }

        repeat(grid.cols + 1) { i ->
            "line" {
                "x1" by i * grid.cellWidth
                "x2" by i * grid.cellWidth
                "y1" by 0
                "y2" by grid.height()
                "stroke" by "black"
                "stroke-dasharray" by 2
